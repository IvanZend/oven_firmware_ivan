...
void SPIClass::begin(uint8_t _pin)
{
  ...
  if ((_pin != CS_PIN_CONTROLLED_BY_USER) && (_spi.pin_ssel == NC)) {
    pinMode(_pin, OUTPUT);
    /*КОСТЫЛЬ, УБРАТЬ ОБРАТНО КОГДА ПЕРЕДЕЛАЕМ ПЛАТУ С ИНВЕРТОРАМИ, СДЕЛАТЬ ПРОСТО digitalWrite(_pin, HIGH);*/
    digitalWrite(_pin, _pin==114?LOW:(_pin==2?LOW:(_pin==57?LOW:(_pin==38?LOW:(_pin==41?LOW:(_pin==78?HIGH:HIGH))))));
  }
...
byte SPIClass::transfer(uint8_t _pin, uint8_t data, SPITransferMode _mode)
{
  ...
  if ((_pin != CS_PIN_CONTROLLED_BY_USER) && (_spi.pin_ssel == NC)) {
    /*КОСТЫЛЬ, УБРАТЬ ОБРАТНО КОГДА ПЕРЕДЕЛАЕМ ПЛАТУ С ИНВЕРТОРАМИ, СДЕЛАТЬ ПРОСТО digitalWrite(_pin, LOW);*/
    digitalWrite(_pin, _pin==114?HIGH:(_pin==2?HIGH:(_pin==57?HIGH:(_pin==38?HIGH:(_pin==41?HIGH:(_pin==78?HIGH:LOW))))));
  }
