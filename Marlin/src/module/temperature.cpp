...
#define TEMPDIR(N) ((HEATER_##N##_RAW_LO_TEMP) < (HEATER_##N##_RAW_HI_TEMP) ? 1 : -1)
  ...
  constexpr temp_range_t sensor_heater_0 { HEATER_0_RAW_LO_TEMP, HEATER_0_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_1 { HEATER_1_RAW_LO_TEMP, HEATER_1_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_2 { HEATER_2_RAW_LO_TEMP, HEATER_2_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_3 { HEATER_3_RAW_LO_TEMP, HEATER_3_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_4 { HEATER_4_RAW_LO_TEMP, HEATER_4_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_5 { HEATER_5_RAW_LO_TEMP, HEATER_5_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_6 { HEATER_6_RAW_LO_TEMP, HEATER_6_RAW_HI_TEMP, 0, 65535 },
                         sensor_heater_7 { HEATER_7_RAW_LO_TEMP, HEATER_7_RAW_HI_TEMP, 0, 65535 };
...
void Temperature::manage_heater() {
  ...
  updateTemperaturesFromRawValues(); // also resets the watchdog
    ...
    if (temp_hotend[0].celsius < _MAX(HEATER_0_MINTEMP, HEATER_0_MAX6675_TMIN + .00)) {
      min_temp_error(H_E0);
    }
  #endif
    ...
    if (temp_hotend[2].celsius > _MIN(HEATER_2_MAXTEMP, HEATER_2_MAX6675_TMAX - 0.0)) max_temp_error(H_E2);
    if (temp_hotend[2].celsius < _MAX(HEATER_2_MINTEMP, HEATER_2_MAX6675_TMIN + .00)) min_temp_error(H_E2);
  #endif
...
#define SCAN_THERMISTOR_TABLE(TBL,LEN) do{                             \
  ...
  for (;;) {                                                           \
    m = (l + r) >> 1;                                                  \
    if (!m) return long(pgm_read_word(&TBL[0][1]));                   \
    if (m == l || m == r) return long(pgm_read_word(&TBL[LEN-1][1])); \
    long v00 = pgm_read_word(&TBL[m-1][0]),                           \
          v10 = pgm_read_word(&TBL[m-0][0]);                           \
         if (raw < v00) r = m;                                         \
    else if (raw > v10) l = m;                                         \
    else {                                                             \
      const long v01 = long(pgm_read_word(&TBL[m-1][1])),            \
                  v11 = long(pgm_read_word(&TBL[m-0][1]));            \
  ...
  float Temperature::analog_to_celsius_hotend(const int raw, const uint8_t e) {
    ...
    switch (e) {
      ...
      case 2:
        ...
        #elif ENABLED(HEATER_2_USES_MAX6675)
          return (
            #if ENABLED(MAX6675_IS_MAX31865)
              max31865.temperature(100, 400)  // 100 ohms = PT100 resistance. 400 ohms = calibration resistor
            #else
      ...
      const long(*tt)[][2] = (long(*)[][2])(heater_ttbl_map[e]);
...
void Temperature::updateTemperaturesFromRawValues() {
  ...
  #endif
  #if ENABLED(HEATER_2_USES_MAX6675)
    temp_hotend[2].raw = READ_MAX6675(2);
  #endif
  #if ENABLED(HEATER_0_USES_ADS1118)
    temp_hotend[0].raw = READ_ADS1118(0);
  #endif
  #if ENABLED(HEATER_1_USES_ADS1118)
    temp_hotend[1].raw = READ_ADS1118(1);
  #endif
  #if ENABLED(HEATER_2_USES_ADS1118)
    ...
    if(temp_hotend[0].raw_count1>=100){ 
      ...
      for(int i=0;i<100;i++){
        if(temp_hotend[0].raw_OLD[i]<temp_hotend[0].raw_MIN)temp_hotend[0].raw_MIN = temp_hotend[0].raw_OLD[i];
        if(temp_hotend[0].raw_OLD[i]>temp_hotend[0].raw_MAX)temp_hotend[0].raw_MAX = temp_hotend[0].raw_OLD[i];
      }
    }
    if(temp_hotend[0].raw_count1>=199){
      temp_hotend[0].raw_count1 = 99;
    }
...
/*
  ...
  * пин дергается в функции 
  * thermalManager.manage_pwm();
  * 
...
void Temperature::disable_all_heaters() {
  ...
  int Temperature::read_max6675(
    ...
    switch (hindex) { 
        case 0:   HAL_GPIO_WritePin(MAX_CS1_GPIO_Port, MAX_CS1_Pin, GPIO_PIN_RESET); break; 
        case 1:   HAL_GPIO_WritePin(MAX_CS3_GPIO_Port, MAX_CS3_Pin, GPIO_PIN_RESET); break; 
        case 2:   HAL_GPIO_WritePin(MAX_CS4_GPIO_Port, MAX_CS4_Pin, GPIO_PIN_RESET); break; 
        default:  break; 
    }
    // HAL_GPIO_WritePin(GPIOA, MAX_CS_Pin, GPIO_PIN_RESET);
    ...
    if (max6675_temp & MAX6675_ERROR_MASK) {
      ...
      // Thermocouple open
      max6675_temp = 4 * (
        #if ALL(HEATER_0_USES_MAX6675, HEATER_1_USES_MAX6675, HEATER_2_USES_MAX6675)
          hindex==0 ? HEATER_0_MAX6675_TMAX : (hindex==1 ? HEATER_1_MAX6675_TMAX : HEATER_2_MAX6675_TMAX)
        #elif BOTH(HEATER_0_USES_MAX6675, HEATER_1_USES_MAX6675)
          hindex==0 ? HEATER_0_MAX6675_TMAX : HEATER_1_MAX6675_TMAX
        #elif BOTH(HEATER_1_USES_MAX6675, HEATER_2_USES_MAX6675)
          hindex==1 ? HEATER_1_MAX6675_TMAX : HEATER_2_MAX6675_TMAX
        #elif BOTH(HEATER_2_USES_MAX6675, HEATER_0_USES_MAX6675)
          hindex==2 ? HEATER_2_MAX6675_TMAX : HEATER_0_MAX6675_TMAX
        #else 
          #if ENABLED(HEATER_0_USES_MAX6675)
  ...
  int Temperature::read_ads1118(
    ...
    #else
      // Needed to return the correct temp when this is called too soon
      static uint16_t ads1118_temp_previous[COUNT_1118] = { 0 };
      static bool switch_input = false;
    #endif

    #define ADS1118_HEAT_INTERVAL 250UL

    static uint16_t ads1118_temp = 2000;
    #define ADS1118_ERROR_MASK    4
    #define ADS1118_DISCARD_BITS  3
    #define ADS1118_SPEED_BITS    2  // (_BV(SPR0)) // clock ÷ 16
...
/* 
 * Temperature::isr() is expected to be called at around 1kHz
 *
 * работает только при 2 экструдерах
 * опрос идёт циклично, на опрос нужно время
 * последним лучше запрашивать тот, который будем в следующий раз получать первым
 * 
 * на будущее -
 * 0x0C - термопара на входах AIN0 и AIN1 
 * 0x3C - термопара на входах AIN2 и AIN3 
 * 
 * 
*/
    /*
    ...
    */
    ...
    if ((check_SPI_enables() == 0) && hindex == 1)
    {
      ...
      if (switch_input)
      {
        ads1118_temp_previous[0] = (int)convRegister_2;
...
HAL_HEAT_TIMER_ISR() {
  HAL_timer_isr_prologue(HEAT_TIMER_NUM);
