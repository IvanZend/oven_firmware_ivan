...
static err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
	...
	{
		  tcp_recved(tpcb, p->tot_len);
		  strncpy(str1,(const char*)(p->payload),p->len);
		//   for(int n=0;n<p->len;n++){
		// 	if(ISEOL(str1[n]))str1[n] = 0;
		//   }
		  str1[p->len+1] = '\0';

		//   char uartstr[50];
		//   sprintf(uartstr,"echo: queue.length = %i \r\n",queue.length);
		//   HAL_UART_Transmit(&huart2, (uint8_t*)uartstr,strlen(uartstr),0x1000);

		  HAL_UART_Transmit(&huart2, (uint8_t*)str1,strlen(str1),0x1000);
		//   if (queue.length < BUFSIZE)queue._enqueue(str1,true);
			pbuf_free(p);
		  ret_err = ERR_OK;
	}
...
void ETHsendchar(char* buf_str)
{
	usartprop.is_text=1;
	tcp_sent(ss->pcb, tcp_server_sent);
	tcp_write(ss->pcb, (void*)buf_str, 1, 1);
	tcp_recved(ss->pcb, 1);
	usartprop.usart_cnt=0;
}
//-----------------------------------------------
