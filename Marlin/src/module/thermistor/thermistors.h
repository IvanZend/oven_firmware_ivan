...
#define THERMISTOR_TABLE_ADC_RESOLUTION 65536
...
// Currently Marlin stores all oversampled ADC values as int32_t, make sure the HAL settings do not overflow 31bit
#if MAX_RAW_THERMISTOR_VALUE > ((1 << 31) - 1)
  #error "MAX_RAW_THERMISTOR_VALUE is too large for int32_t. Reduce OVERSAMPLENR or HAL_ADC_RESOLUTION."
#endif

#define OV_SCALE(N) (N)
#define OV(N) int32_t(OV_SCALE(N) * (OVERSAMPLENR) * HAL_ADC_RANGE / (THERMISTOR_TABLE_ADC_RESOLUTION))
...
#endif
#if ANY_THERMISTOR_IS(996) // User-defined table for TOTALZ pt100 (2)
  #include "thermistor_996.h"
#endif
#if ANY_THERMISTOR_IS(997) // User-defined table for TOTALZ pt100 (1)
