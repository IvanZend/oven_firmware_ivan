...
// extern int 				                    USBstatusOld;
// extern int 				                    USBstatusNew;
/* constructor */
USB::USB() : bmHubPre(0) {
...
void USB::Task() { //USB state machine
  ...
  //usb_result = f_mount(&USBDISKFatFs, (TCHAR const*)USBHPath, 0);
  MX_USB_HOST_Process();
  //tmpdata = SE1;
...
// FR_OK
  ...
  switch (Appli_state) {
    case APPLICATION_START:
      ...
      */
      break; //illegal state
    case APPLICATION_IDLE: //illegal state
      usb_task_state = USB_DETACHED_SUBSTATE_ILLEGAL;
      lowspeed = false;
      break;
    case APPLICATION_DISCONNECT: //disconnected
    ...
    case APPLICATION_READY: //attached
      if ((usb_task_state & USB_STATE_MASK) == USB_STATE_DETACHED) {
