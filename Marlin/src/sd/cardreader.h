...
#define MAX_DIR_DEPTH     255//10       // Maximum folder depth
#define MAXDIRNAMELENGTH   255//8       // DOS folder name size
#define MAXPATHNAMELENGTH  255//(1 + (MAXDIRNAMELENGTH + 1) * (MAX_DIR_DEPTH) + 1 + FILENAME_LENGTH) // "/" + N * ("ADIRNAME/") + "filename.ext"
...
extern 		FIL 				FileToRender;                   							/* File object */
...
public:
  ...
  static void select_to_print(uint16_t bt);
  static void ls(bool display, int pos);

  static size_t shortlen(TCHAR * str);
  static void   tchartochar(char * ch, TCHAR * str);
  static unsigned short * tcharconv(unsigned short tostr[], unsigned short fromstr[], uint8_t start, uint16_t endsign);
  // SD Card Logging
  ...
  // Basic file ops
  static void openFileRender(char * const path, TCHAR * const altpath=NULL, const uint8_t subcall=0);
  static void openFileRead(char * const path, TCHAR * const altpath=NULL, const uint8_t subcall=0);
  ...
  static void selectFileByName(const TCHAR* const match);

  // Print job
  static void openAndPrintFile(const char *name);   // (working directory)
  static void fileHasFinished();
  static void getAbsFilename(TCHAR *dst);
  ...
  static inline uint8_t percentDone() { return (FileFromSD.obj.objsize) ? sdpos / ((filesize + 99) / 100) : 0; }
  ...
  static inline bool eof() { return sdpos >= FileFromSD.obj.objsize; }
  static inline bool eor() { return sdpos >= FileToRender.obj.objsize; }
  ...
  static void reset_page(){cur_page=1;};
  static void increment_page(){cur_page++;};
  static void decrement_page(){cur_page--;};
  static void stop_sd_print();
...
private:
  ...
  static uint32_t rendersize, rsdpos;
  ...
  static TCHAR proc_filenames[SD_PROCEDURE_DEPTH][MAXPATHNAMELENGTH];
  ...
  // Directory items
  //
  // static TCHAR fileNames[255][255];
  static bool is_dir_or_gcode(const dir_t &p);
  static bool is_dir_or_gcode_ff(FILINFO* fno);/* Pointer to file information to return */
  static int countItems(SdFile dir);
  static void selectByIndex(SdFile dir, const uint8_t index);
  static void selectByName(SdFile dir, const TCHAR * const match);
  static void printListing(SdFile parent, const char * const prepend=nullptr);
  static FRESULT scan_files(int* num,bool display, int pos, TCHAR* path, char* debug, TCHAR * prepend=nullptr);
