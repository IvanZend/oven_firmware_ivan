...
int16_t SdBaseFile::read(void* buf, uint16_t nbyte) {
  ...
  NOMORE(nbyte, FileFromSD.obj.objsize - curPosition_);


  /*****************************************************************/
	FRESULT res;
  if(nbyte == 0)return 0;
	uint8_t readData[nbyte];
	unsigned int ByteRead = 0;

  toRead = nbyte;

  res = f_lseek (&FileFromSD, curPosition_);	
  ...
  while (toRead > 0) {
    ...
    res = f_read(&FileFromSD, readData, nbyte, &ByteRead);
    if(res != FR_OK)return -1;
    uint8_t* src = readData;
    memcpy(dst, src, n);  
...
int16_t SdBaseFile::write(const void* buf, uint16_t nbyte) {
  #if (ENABLED(SDCARD_READONLY) || ENABLED(_FS_READONLY))
  ...
  FRESULT res;
  if(curPosition_>FileFromSD.obj.objsize)curPosition_ = 0;
  res = f_lseek(&FileFromSD,curPosition_);
  if(res != FR_OK) goto FAIL;
  res = f_write(&FileFromSD,buf,nbyte,&BytesWritefile);
  if(res != FR_OK) goto FAIL;
...
int16_t SdBaseFile::write_w25(const void* buf, uint16_t nbyte, uint8_t card_sel, uint16_t sector) {
  // convert void* to uint8_t*  -  must be before goto statements
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	// максимальный размер для записи - [страница*размер_страницы - 4 байта резерв под размер записываемого]
	if(n>(0x7FFFFF * 256 - 4)){
    // writeError = true;
    return -1;
  }//goto FAIL;///*7FFFFF, but 1 byte for size of sizelength, 3 bytes for sizelength at maximum*////

  uint32_t 	addr 			= 4096 * sector;//0x000000;адрес в страницах, в секторе 4096 страниц, в странице 256 байт
  // uint32_t 	startPage 		= (addr & 0xffff00) >> 8;
  // int 		startOffset 	= (addr & 0x0000ff);

  char uintToHex[50];
  uint8_t readBuff[256];
  //заполняем первые байты первого 256 буфера - 1 байт сколько размер + байты размера сколько указано
  uint8_t firstSize = getFirstSize(readBuff,n);
  //дозаполняем буфер до конца 256, firstsize - сколько осталось места в буфере 256 под данные
  //аргументы - буфер для заполнения, размер файла (пока не используется), начало буфера для заполнения, сколько читаем, оффсет
  // read256fromUSB(readBuff,n,256-firstSize,firstSize,0);
  for(int b=256-firstSize;b<256;b++){
    if(b>=(n+256-firstSize)){
      readBuff[b] = 0xFF;
    }else{
      readBuff[b] = *src;
      *src++;
  ...
  /*--------------------------------------------------------------------*/
  // для записи нужно стереть сектор либо блок
  // рассчитываем размер для стирания - минимум 4096 сектор, 65536 блок
  for(int s=sector;s<sector+1+n/4096;s++){
    W25qxx_EraseSector(s);
  }

  W25qxx_WritePage(readBuff,addr,0,sizeof(readBuff));

  int counter = addr;
  for(int t=getFirstSize(readBuff,n);t<n;t=t+256){
    if((n-t)>=256){
      for(int b=0;b<256;b++){
        readBuff[b] = *src;
        *src++;
    ...
    ////////////////////////////////////////////////////////
    // uint8_t flsizbuf[10];
    // W25qxx_ReadBytes(flsizbuf,addr,10);
    ////////////////////////////////////////////////////////
    W25qxx_WritePage(readBuff,counter,0,sizeof(readBuff));

  }
  // sprintf(uintToHex,"\r\ni counted: %d pages",counter);
  // USART_SendToCom(&uintToHex);
  /*--------------------------------------------------------------------*/

  uint8_t readBuf1[100];
  W25qxx_ReadBytes(readBuf1,addr*256,100);
  sprintf(uintToHex,"100 bytes: %h \r\n",readBuf1);
