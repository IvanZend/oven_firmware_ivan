...
extern        uint8_t                           sdfile_selected;
extern        TCHAR                           sdfiles[15][255];
#if HAS_DISPLAY
  #include "../LTDC/ltdc.h"
...
millis_t MarlinUI::edited_timer_ms; // = 0
 
bool MarlinUI::delta_z_available_old;
bool MarlinUI::delta_z_available_new;
...
// Encoder Handling
  ...
  screenFunc_t MarlinUI::prevScreen; // Initialized in CTOR  
  bool MarlinUI::DrawPhaseOne; // Initialized in CTOR
  bool MarlinUI::DrawPhaseTwo; // Initialized in CTOR
  bool MarlinUI::iconsReady; // Initialized in CTOR
  uint8_t MarlinUI::hotendActive; // Initialized in CTOR
  uint8_t MarlinUI::percent = 0; // Initialized in CTOR
  int16_t MarlinUI::edited_settings; // Initialized in CTOR
  bool MarlinUI::hotendActive0; // Initialized in CTOR
  bool MarlinUI::hotendActive1; // Initialized in CTOR  
  CALeditState MarlinUI::calibstate; // Initialized in CTOR
  CALeditState MarlinUI::calibstate_old; // Initialized in CTOR
  bool MarlinUI::bedActive; // Initialized in CTOR
  bool MarlinUI::chamberActive; // Initialized in CTOR
  bool MarlinUI::screen_changed;
  uint8_t MarlinUI::opt_disctance;
  ...
  #if ENABLED(TOUCH_BUTTONS)
    uint16_t MarlinUI::touch_buttons;
...
screenFunc_t MarlinUI:: get_screen() {
...
void MarlinUI::status_screen() {
  ...
  //status_screen_refresh();
    ...
    if (use_click()) {
        ...
        // TFT_LoadFromLayer(MENU_LAYER1,MAIN_LAYER);
  ...
  if (!oven_display.draw_all_completed)       // если ранее не отрисовывали весь дисплей
  {
    ...
    oven_display.widgets_vector[33].check_slider_lock();
...
void MarlinUI::kill_screen(PGM_P lcd_error, PGM_P lcd_component) {
  ...
  HAL_GPIO_WritePin(BLUE_PWM_GPIO_Port, BLUE_PWM_Pin, GPIO_PIN_RESET);
  // RED ALERT. RED ALERT.
  #ifdef LED_BACKLIGHT_TIMEOUT
...
void MarlinUI::update() {
  ...
  millis_t ms = millis();
    ...
    // screenFunc_t curScreen = currentScreen;
    // update_buttons();
      ...
      if (sd_status) {
        ...
        if(card.isMounted()){
          if (old_sd_status == 2)
            ...
            if(currentScreen == status_screen)LTDC_IconFromLayer(58, 507, 61, 43,TTZ_COL_ORAN, BOTTOM_PANEL_FOLDER_START, FLASH_LAYER1, MAIN_LAYER,0,0);
          }
        }
      }
      #if PIN_EXISTS(SD_DETECT)
          ...
          if (old_sd_status != 2) {
            ...
            if(currentScreen == status_screen)LTDC_IconFromLayer(58, 507, 61, 43,TTZ_COL_D_GR, BOTTOM_PANEL_FOLDER_START, FLASH_LAYER1, MAIN_LAYER,0,0);
            if(currentScreen == menu_sdmain){
              goto_screen(status_screen);
              home_sd_rewrite();
              LTDC_IconFromLayer(58, 507, 61, 43,TTZ_COL_D_GR, BOTTOM_PANEL_FOLDER_START, FLASH_LAYER1, MAIN_LAYER,0,0);
            }
            // #if HAS_LCD_MENU
            //   return_to_status();
            // #endif
  ...
  ) 
  {
    //main_device.process_timer_left.seconds_timer_handler();
      ...
      //   queue.advance();
      //   queue.inject_P(PSTR("G1 X250 Y250 F9000"));
      //   queue.advance();
    ...
    next_lcd_update_ms = ms + LCD_UPDATE_INTERVAL;
      ...
      #if HAS_ENCODER_ACTION
        // touch_buttons = touch.returnBtn(currentScreen);
      #endif
    ...
    // uint16_t bbr2 = planner.block_buffer_runtime() >> 1;
      ...
      //   }
        // if (do_u8g_loop) {
          ...
          //     // TFT_LoadFromLayer(BUFFER_LAYER, MAIN_LAYER);
          //   }else{
          //     // TFT_LoadFromLayer(MENU_LAYER1, MAIN_LAYER);
          //   }
          //   // TFT_LoadFromLayer(MENU_LAYER1, MAIN_LAYER);
  ...
  void MarlinUI::set_status(const char * const message, const bool persist) {
    ...
    const char* pend = message + UTF_strlen(message);
  ...
  void MarlinUI::set_status_P(PGM_P const message, int8_t level) {
    ...
    if(strcmp(message,status_message) != 0){      
      status_new = 1;
    }
