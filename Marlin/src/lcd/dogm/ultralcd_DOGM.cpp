...
bool MarlinUI::detected() { return true; }
  ...
  void MarlinUI::draw_marlin_bootscreen(const bool line2/*=false*/) {
    ...
    if (text_max_width < rspace) {
      ...
      offx = (width - text_max_width - (START_BMPWIDTH)) / 3;                             // First the boot logo...
  ...
  void MarlinUI::show_marlin_bootscreen() {
    ...
    for (uint8_t q = pages; q--;) {
      ...
      TFT_LoadFromLayer(MAIN_LAYER,MAIN_LAYER);
...
void MarlinUI::draw_kill_screen() {
  // uint16_t len0 = UTF_strlen(status_message);
  uint8_t len1 = UTF_strlen(status_message);
  uint8_t len2 = UTF_strlen(GET_TEXT(MSG_HALTED));
  uint8_t len3 = UTF_strlen(GET_TEXT(MSG_PLEASE_RESET));
  uint16_t w1 = len1*lcdprop.pFont[2]->Width;
  uint16_t w2 = len2*lcdprop.pFont[2]->Width;
  uint16_t w3 = len3*lcdprop.pFont[2]->Width;
  uint16_t width = lcdprop.pFont[2]->Width*(len1 > len2 ? (len1 > len3 ? len1 : (len2 > len3 ? len2 : len3)): (len2 > len3 ? len2 : (len1 > len3 ? len1 : len3)));
  TFT_DrawRectangle((hltdc.LayerCfg[0].ImageWidth-(width+100))>>1, 200,width+100, 200, 0,LCD_COLOR_RED,0x0, MAIN_LAYER, 101,1,0);
  TFT_DrawText(TFT_FONT16,hltdc.LayerCfg[0].ImageWidth/2 - w1/2, 250, status_message,0,MAIN_LAYER, MAIN_LAYER,LCD_COLOR_RED);
  TFT_DrawText(TFT_FONT16,hltdc.LayerCfg[0].ImageWidth/2 - w2/2, 300, GET_TEXT(MSG_HALTED),0,MAIN_LAYER, MAIN_LAYER,LCD_COLOR_RED);
  TFT_DrawText(TFT_FONT16,hltdc.LayerCfg[0].ImageWidth/2 - w3/2, 350, GET_TEXT(MSG_PLEASE_RESET),0,MAIN_LAYER, MAIN_LAYER,LCD_COLOR_RED);
  // #if ENABLED(LIGHTWEIGHT_UI)
  ...
  // do {
  //   // set_font(FONT_MENU);
  //   TFT_DrawText(100, 400, status_message,0,MAIN_LAYER, 1,0x0);
  //   TFT_DrawText(100, 450, GET_TEXT(MSG_HALTED),0,MAIN_LAYER, 1,0x0);
  //   TFT_DrawText(100, 500, GET_TEXT(MSG_PLEASE_RESET),0,MAIN_LAYER, 1,0x0);
  //   // lcd_put_u8str(100, h4 * 1, status_message);
  //   // lcd_put_u8str_P(100, h4 * 2, GET_TEXT(MSG_HALTED));
  //   // lcd_put_u8str_P(100, h4 * 3, GET_TEXT(MSG_PLEASE_RESET));
  // } while (u8g.nextPage());
...
void MarlinUI::clear_lcd(uint8_t layer) { 
 TFT_FillScreen(TTZ_COL_BAGR,layer);//black screen
  ...
  void MenuItemBase::_draw(const bool sel, const uint8_t row, PGM_P const pstr, const char, const char post_char) {
    ...
    // TFT_FillScreen(LCD_COLOR_RANDOM,MAIN_LAYER);
	
    /******************************
	TFT_DrawMenuItem(row,pstr,BUFFER_LAYER,sel ? LCD_COLOR_GREEN : LCD_COLOR_RED,0x0);
	*******************************/
