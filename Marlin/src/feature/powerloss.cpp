...
void PrintJobRecovery::check() {
  ...
  // FL_sel = 2;
  // uint8_t l_size[1];
  // W25qxx_ReadByte(l_size,0);
  // if(l_size[0]!=0xFF){
    load();
    if (!valid()) return purge();
    queue.inject_P(PSTR("M1000 S"));
  // }
  // uint8_t length[l_size[0]];
  // W25qxx_ReadBytes(length,0,l_size[0]);
...
int16_t PrintJobRecovery::read(void* buf, uint16_t nbyte){

  uint8_t* dst = reinterpret_cast<uint8_t*>(buf);
  ///////////////////////////////////////////////////////////////////////////////////
  uint8_t rec_length[1];
  W25qxx_ReadByte(rec_length,0);
  uint8_t rec_size[rec_length[0]];
  W25qxx_ReadBytes(rec_size,1,rec_length[0]);

  uint32_t data_size = 0;
  if(rec_length[0] == 1){
    data_size = rec_size[0];
  }else if(rec_length[0] == 2){
    data_size = rec_size[0]<<8 | rec_size[1];
  }else if(rec_length[0] == 3){
    data_size = rec_size[0]<<16 | rec_size[1]<<8 | rec_size[2];
  }else{
    return -1;
  }
  if(data_size != nbyte)return -1;
  uint8_t rec_data[data_size];
  W25qxx_ReadBytes(rec_data,1+rec_length[0],data_size);

  uint8_t* src = rec_data;
...
void PrintJobRecovery::write() {
  ...
  open(false);
  // file.seekSet(0);
  const int16_t ret = file.write_w25(&info, sizeof(info), 2, 0);
...
void PrintJobRecovery::resume() {
  ...
  TCHAR *fn = info.sd_filename;
  // extern const char M23_STR[];
  // sprintf_P(cmd, M23_STR, fn);
  card.openFileRead(NULL,info.sd_filename);
  // gcode.process_subcommands_now(cmd);
